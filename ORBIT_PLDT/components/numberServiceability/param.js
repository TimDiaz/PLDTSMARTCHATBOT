...
module.exports = {
    ...
    invoke: (conversation, done) => {
        ...
        logger.sendEmail = ((result, resultCode) => {
            process.ParamEmailSender(result, resultCode, serviceNumber, accountNumber);
        })

        logger.start = (() => {
            process.ParamLoggerStart();
        });

        logger.end = (() => {
            process.ParamLoggerEnd(transition);
            _logger.shutdown();
        ...
        api.PostRequest(areacode, telephone, (error, response) => {
            if (error) {
                ...
                const result = process.ParamProcess(response.statusCode, response.body, accountNumber, serviceNumber)
