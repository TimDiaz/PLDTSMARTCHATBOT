...
module.exports = {
    ...
    invoke: (conversation, done) => {
        ...
        logger.sendEmail = ((result, resultCode) => {
            process.RegionEmailSender(result, resultCode, serviceNumber, accountNumber);
        })

        logger.start = (() => {
            process.RegionLoggerStart();
        });

        logger.end = (() => {
            process.RegionLoggerEnd(transition);
            _logger.shutdown();
        ...
        logger.debug(`[Service Number] ${serviceNumber}`)
        ...
        api.PostRequest(areacode, telephone, (error, response) => {
            if (error) {
                ...
                const result = process.RegionProcess(response.statusCode, response.body, accountNumber, serviceNumber)
