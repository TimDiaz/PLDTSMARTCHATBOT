...
module.exports = {
    metadata: () => ({
        name: componentName.BSMPTimer.FIBR,
        properties: {
            Mobile: {
                type: "string",
                required: false
    ...
    invoke: (conversation, done) => {
        ...
        logger.start = () => {
            logger.info(`-------------------------------------------------------------------------------------------------------------`)
            logger.info(`- [START] BSMP Timer - [FIBR]                                                                               -`)
            logger.info(`-------------------------------------------------------------------------------------------------------------`)
        }

        logger.end = () => {
            logger.info(`-------------------------------------------------------------------------------------------------------------`)
            logger.info(`- [END] BSMP Timer - [FIBR]                                                                                 -`)
            logger.info(`-------------------------------------------------------------------------------------------------------------`)
        ...
        var Process = () => {
            ...
            const optionsRT = globalProp.BSMPTimer.API.UpdatePostOptions(telNumber);
            logger.info(`[SOAP REAL TIME REQUEST OPTION] ${JSON.stringify(optionsRT)}`);
            request(optionsRT, function (errorRT, responseRT) {
                if (errorRT) {
                    logger.error(`[SOAP REAL TIME ERROR] ${JSON.stringify(errorRT)}`);
                    ...
                    if (responseRT.statusCode > 200) {
                        InsertData(accNumber, telNumber, smpStartTs, smpNeType, "NO RESPONSE", retry);
                        logger.error(`[SOAP REAL TIME ERROR] ${responseRT.body}`);
                        logger.end();
                    } else {
                        const options = globalProp.BSMPTimer.API.PostOptions(telNumber);
                        logger.info(`[SOAP PE DATA REQUEST OPTION] ${JSON.stringify(options)}`);
                        request(options, function (error, response) {
                            if (error) {
                                logger.error(`[SOAP PE DATA ERROR] ${JSON.stringify(errorRT)}`);
                                ...
                                if (response.statusCode > 200) {
                                    InsertData(accNumber, telNumber, smpStartTs, smpNeType, "NO RESPONSE", retry);
                                    logger.error(`[SOAP PE DATA ERROR] ${response.body}`);
                                    logger.end();
                                }
                                else {
                                    var extractedData = "";
