...
const getAppender = (name, config) => {
  console.log(`Has appender name: ${appenders.has(name)}`);
...
const setup = (config) => {
  ...
  Object.values(config.categories).forEach((category) => {
    console.log(`push used appenders ${category.appenders}`);
  ...
  Object.keys(config.appenders).forEach((name) => {
    ...
    console.log(name);
    console.log(config.appenders[name]);
    if (
      usedAppenders.includes(name) ||
      config.appenders[name].type === 'tcp-server' ||
      config.appenders[name].type === 'wss-server' ||
